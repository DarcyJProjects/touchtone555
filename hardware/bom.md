# BOM– TouchTone555

This file lists the Bill of Materials (BOM) - the components required to complete this project.

---

## Semiconductors

| Qty | Part             | Notes                 |
| --- | ---------------- | --------------------- |
| 2   | 1N5817           | Schottky diode, DO-41 |
| 1   | 2N3906           | PNP, TO-92            |
| 1   | 2N3904           | NPN, TO-92            |
| 1   | BC548            | NPN, TO-92            |
| 1   | LM386            | DIP-8                 |
| 1   | NE555P           | DIP-8                 |
| 1   | LED (any colour) | 3 mm                  |

### Resistors

| Qty | Value  |
| --- | ------ |
| 1   | 100 Ω  |
| 1   | 330 Ω  |
| 1   | 1 kΩ   |
| 2   | 10 kΩ  |
| 2   | 68 kΩ  |
| 1   | 4.7 kΩ |
| 1   | 10 MΩ  |

### Potentiometers / Trimmers

| Qty | Type                             | Value / Model | Notes              |
| --- | -------------------------------- | ------------- | ------------------ |
| 20  | Trimmer (3296W)                  | 5 kΩ          |                    |
| 1   | Trimmer (3296W)                  | 10 kΩ         |                    |
| 1   | Rotary Potentiometer (A5K RK097) | 5 kΩ          | **See Misc Parts** |

### Capacitors

| Qty | Value  | Type / Notes                                                                    |
| --- | ------ | ------------------------------------------------------------------------------- |
| 15  | 10 nF  | Green polyester capacitors (rectangular) - do not use ceramic (poor stability). |
| 8   | 100 nF | Ceramic disc                                                                    |
| 1   | 10 µF  | Electrolytic, radial                                                            |
| 1   | 100 µF | Electrolytic, radial                                                            |

### Connectors and Jacks

| Qty | Part           | Description                                |
| --- | -------------- | ------------------------------------------ |
| 1   | DC Barrel Jack | **See Misc Parts**                         |
| 3   | JST-PH 2-pin   | Stylus, speaker, and 9 V battery connector |
| 1   | PJ-320A        | 3.5 mm audio jack **See Misc Parts**       |

### Switches

| Qty | Part                           | Description                                 |
| --- | ------------------------------ | ------------------------------------------- |
| 2   | 2 Position Slide Switch (SPDT) | Power and vibrato switch **See Misc Parts** |
| 1   | 4 Position Slide Switch (2P4T) | Octave switch **See Misc Parts**            |

### Speaker

| Qty | Part    | Description                      |
| --- | ------- | -------------------------------- |
| 1   | Speaker | 8 Ω, 2W 40 mm **See Misc Parts** |





---

> 💡 *Note:* Compare parts with the build photos, schematic, and PCB layout to check for correct footprints and sizes. The components listed here are only suggestions and are not guaranteed. Check misc parts file for a detailed look at the less standard components.
